...
<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="RuleEdit.aspx.cs" Inherits="Settings_RuleEdit" %>
...
// <![CDATA[
        ...
        $(document).ready(function () {
            ...
            $("#gridConditionList tr td :input[class='txtPointField']").each(function () {

                var txtPointField_ID = $(this).attr("id");
                var hdnPointField_ID = txtPointField_ID.replace("txtPointField", "hdnPointField");

                // add auto complete
                AddAutoComplete(txtPointField_ID, hdnPointField_ID);
            });
        ...
        function AddValidators() {
            ...
            //#region add validate to tolerance

            $("#gridConditionList tr td :input[class='txtTolerance']").each(function () {

                if ($(this).attr("maxlength") == 8) {

                    $(this).onlypressnum();
                }

                //alert($(this).parent().parent().find(":input[id='ddlCondition']").val());
                var condition = $(this).parent().parent().find(":input[id='ddlCondition']").val();
                //alert("Condition=" + condition);
                //if ($(this).parent().parent().find(":input[id='ddlCondition']").val() != "1" && $(this).parent().parent().find(":input[id='ddlCondition']").val() != "2") {
                if (condition != "1" && condition != "2" && condition != "13" && condition != "14") {
                    $(this).rules("add", {
                        required: true,
                        messages: {
...
//                }
        ...
        function BeforeDelete() {
            ...
            var IsRef = $("#hdnIsRef").val();
            if (IsRef == "True") {

                var r1 = confirm("The Rule has been referenced by Rule Groups and Loan Rules. Deleting this Rule will remove the references and is not reversible. Are you sure you want to continue?");
                if (r1 == false) {
            ...
            var r2 = confirm("The operation is not reversible. Are you sure you want to continue?");
            if (r2 == false) {
        ...
        function btnClone_onclick() {
            ...
            var RuleID = GetQueryString1("RuleID");
            var RadomNum = Math.random();
            var RadomStr = RadomNum.toString().substr(2);

            var iFrameID = GetQueryString1("iframe");
            if (iFrameID == "") {

                $(window.parent.document).find("#ifrRuleEdit").attr("src", "RuleClone.aspx?sid=" + RadomStr + "&RuleID=" + RuleID + "&CloseDialogCodes=window.parent.CloseDialog()");
            }
            else {

                $(window.parent.document).find("#" + iFrameID).attr("src", "RuleClone.aspx?sid=" + RadomStr + "&RuleID=" + RuleID + "&CloseDialogCodes=window.parent.CloseDialog()");
            }
        }

        //#region add by peter 20110123
        function updateSuccessfully(sReturn) {
            $('#divContainer').hide();
            alert('Update rule successfully.');
            if ('<%=strNeedReturn %>' === "1" && typeof (window.parent.ruleUpdated) === "function") {
                window.parent.ruleUpdated(sReturn);
            }
            else
                window.parent.location.href = window.parent.location.href;
        }

        function deleteSuccessfully(sId) {
            $('#divContainer').hide();
            alert('Delete rule successfully.');
            if ('<%=strNeedReturn %>' === "1" && typeof (window.parent.ruleDeleted) === "function") {
                window.parent.ruleDeleted(sId);
            }
            else
                window.parent.location.href = window.parent.location.href;
        }
        ...
        function Cancel() {
            ...
            if (CloseDialogCodes == "") {
                ...
                window.parent.CloseDialog_EditRule();
            }
            else {
        ...
        function ddlCondition_onchange(this_select) {

            //alert($(this_select).val());

            if ($(this_select).val() == "1" || $(this_select).val() == "2" || $(this_select).val() == "13" || $(this_select).val() == "14") {

                $(this_select).parent().parent().find(":input[id^='txtTolerance']").rules("remove", "required");
            }
            else {

                // add validate
                $(this_select).parent().parent().find(":input[id^='txtTolerance']").rules("add", {
...
<%--             <table cellpadding="0" cellspacing="0" style="margin-top: 15px;">
        ...
        <div id="divCondition" style="margin-top: 25px;">
            ...
            <div id="divConditionList" class="ColorGrid" style="margin-top: 5px;">
                <asp:GridView ID="gridConditionList" runat="server" EmptyDataText="There is no condition, please add." AutoGenerateColumns="False" CellPadding="4" CssClass="GrayGrid" GridLines="None">
                    ...
                    <Columns>
                        ...
                        <asp:TemplateField HeaderText="Sequence" ItemStyle-Width="55px">
                            <ItemTemplate>
                        ...
                        <asp:TemplateField HeaderText="Point Field">
                            <ItemTemplate>
                                <input id='txtPointField<%# Eval("Sequence") %>' name='txtPointField<%# Eval("Sequence") %>' type='text' value="<%# Eval("Label") %>" class='txtPointField' style='width:280px;' />
                                <input id='hdnPointField<%# Eval("Sequence") %>' type='hidden' class='hdnPointField' value="<%# Eval("PointFieldId") %>" />
                            </ItemTemplate>
                        </asp:TemplateField>
                        <asp:TemplateField HeaderText="Condition" ItemStyle-Width="170px">
                            <ItemTemplate>
                                <select id="ddlCondition" style="width: 150px;" onchange="ddlCondition_onchange(this)">
                                    <%# this.GetOptions_ddlCondition(Eval("DataType").ToString(), Eval("Condition").ToString()) %>
                                </select>
                            </ItemTemplate>
                        </asp:TemplateField>
                        <asp:TemplateField HeaderText="Tolerance" ItemStyle-Width="150px">
                            <ItemTemplate>
                                <%# this.GetToleranceInput(Eval("Sequence").ToString(), Eval("DataType").ToString(), Eval("Tolerance").ToString())%>
                            </ItemTemplate>
                        </asp:TemplateField>
                        <asp:TemplateField HeaderText="Type" ItemStyle-Width="50px">
                            <ItemTemplate>
                                <%# this.GetToleranceTypeDropdownList(Eval("DataType").ToString(), Eval("ToleranceType").ToString())%>
                            </ItemTemplate>
                        </asp:TemplateField>
                    </Columns>
                </asp:GridView>
                    ...
                    <table id="gridConditionList1" class="GrayGrid" cellspacing="0" cellpadding="4" style="border-collapse: collapse;">
                        ...
                        <tr>
	                        ...
	                        <td style="width: 170px;">
                                <select id="ddlCondition" title="Please select Point Field first." style="width: 150px;" disabled onchange="ddlCondition_onchange(this)">
    ...
    <div id="divHiddenFields" style="display: none">
        <input id="hdnCounter" runat="server" type="text" value="0" />
        ...
        <asp:TextBox ID="hdnIsRef" runat="server"></asp:TextBox>
