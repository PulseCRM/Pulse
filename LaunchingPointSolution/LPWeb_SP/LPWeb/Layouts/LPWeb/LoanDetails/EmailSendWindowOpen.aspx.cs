...
public partial class LoanDetails_EmailSendWindowOpen : BasePage
{
    int iLoanID = 0;
    int iProspectID = 0;
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
            ...
            && this.Request.QueryString["ProspectAlertID"] == null)
        {
            ...
            this.ClientScript.RegisterStartupScript(this.GetType(), "_InvalidQuery1", "$('#divContainer').hide();alert('Missing required query string.');window.close();", true);
            return;
        }
        ...
        if (this.Request.QueryString["LoanID"] != null)
        {
            ...
            if (bIsValid == false)
            {
                ...
                this.ClientScript.RegisterStartupScript(this.GetType(), "_InvalidQuery2", "$('#divContainer').hide();alert('Missing required query string.');window.close();", true);
                return;
            }
        ...
        else if (this.Request.QueryString["ProspectAlertID"] != null)
        {
            ...
            {
                this.ClientScript.RegisterStartupScript(this.GetType(), "_InvalidQuery3", "$('#divContainer').hide();alert('Invalid prospect alert id.');window.close();", true);
                return;
            }
    ...
    protected void btnSend_Click(object sender, EventArgs e)
    {
        ...
        {
            PageCommon.WriteJsEnd(this, "Sent email successfully.", "window.close();");
        }
        else
        {
            PageCommon.WriteJsEnd(this, "Failed to send email: " + response.resp.StatusInfo.Replace("'", "\'"), "window.close();");
        }
    }
}
