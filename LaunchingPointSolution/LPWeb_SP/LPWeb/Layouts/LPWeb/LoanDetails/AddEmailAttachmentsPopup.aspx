...
<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="AddEmailAttachmentsPopup.aspx.cs" Inherits="AddEmailAttachmentsPopup" %>
<!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd">
<html xmlns="http://www.w3.org/1999/xhtml">
<head id="Head1" runat="server">
    <title>Add Email Attachments Popup</title>
                  ...
                  if(id.indexOf("s") ==-1 )
                    {
                        ...
                        alert("Mail template attachment, you can not delete it here.");
                        return false;
                    }
		   ...
		   .change(function(){
               ...
               if($(this).val()!="" )
               {
                   ...
                   if(  $(this).val().toLowerCase().indexOf(".pdf") > 0 || $(this).val().toLowerCase().indexOf(".doc") > 0 
                            || $(this).val().toLowerCase().indexOf(".docx") > 0|| $(this).val().toLowerCase().indexOf(".xls") > 0|| $(this).val().toLowerCase().indexOf(".xlsx") > 0
                            || $(this).val().toLowerCase().indexOf(".zip") > 0|| $(this).val().toLowerCase().indexOf(".gif") > 0|| $(this).val().toLowerCase().indexOf(".jpg") > 0
                            || $(this).val().toLowerCase().indexOf(".png") > 0
                             )  //*.PDF, *.Doc, *.DocX, *.XLS, *.XLSX,*.ZIP,*.GIF,*.JPEG,*.PNG
				    {
                        var fileNameList  = $(this).val().split("\\");
                        
					    $("#lbxAttachments").append('<option value="'+upfilecount +'" >'+fileNameList[fileNameList.length-1]+'</option>');
                    ...
                    {
                        alert("File type does not match(*.PDF, *.Doc, *.DocX, *.XLS, *.XLSX,*.ZIP,*.GIF,*.JPEG,*.PNG)!");   
                    }
               }
           })
           .appendTo("#upfilePool");
...
<body>
    ...
    <div id="divDetail">
        <div id="divButtons" class="opt">
            <input ID="btnSend"  type="button" value="Save" class="Btn-91" />
        ...
        <table class="detail">
            <tr>
                ...
                <td>
                    <select id="lbxAttachments" name="lbxAttachments" multiple="multiple" style="color:black; background-color:white; width:300px; height:150px; border:1px solid #bbb; " >
                        <%=Default_lbxAttachments %>
                    </select>
                </td>
                <td>
