...
public partial class BorrowerInfotab : BasePage
{
    int iLoanId = 0;    // 0: Create Lead, >0: Edit Lead
    int iContactId = 0;

    int Global_BorrowerID = 0;
    int Global_CoBorrowerID = 0;
        /// <summary>
    ...
    {
        #region 校验页面参数
        if ((CurrUser.userRole == null) || string.IsNullOrEmpty(CurrUser.userRole.Prospect))          //!(CurrUser.userRole.Prospect.ToUpper().Contains("D")))
        {
            Response.Redirect("../Unauthorize1.aspx");
            return;
        }

        if (!CurrUser.userRole.Prospect.ToUpper().Contains("D"))
            this.btnSaveGoToDetail.Enabled = false;
        ...
        if (this.Request.QueryString["LoanId"] != null && this.Request.QueryString["ContactId"] == null)     // edit lead
        {
            ...
            this.hdnLoanId.Value = this.iLoanId.ToString();
            #endregion
        ...
        else if (this.Request.QueryString["ContactId"] != null && this.Request.QueryString["LoanId"] == null)
        {
            ...
            if (ContactLoanInfo.Rows.Count > 0)
            {
                ...
                this.hdnLoanId.Value = iFileId.ToString();
        ...
        if (this.IsPostBack == false)
        {
            ...
            if (this.sActionMode == "Edit_HasLoanId_NoContactId" || this.sActionMode == "Edit_HasContactId_HasLoan")
            {
                ...
                this.txtPropertyValue.Text = this.CurrentLoanInfo.Rows[0]["SalesPrice"] == DBNull.Value ? string.Empty : Convert.ToDecimal(this.CurrentLoanInfo.Rows[0]["SalesPrice"]).ToString("n2");
    ...
    private DataTable GetProspectLoanInfo(string sLoanId)
    {
        ...
        string sSql = "select * from Loans where FileId=" + sLoanId;
    ...
    private void BindingData_Borrower(DataTable LoanInfoData)
    {
        ...
        if (this.BorrowerInfo_Contact != null && this.BorrowerInfo_Contact.Rows.Count > 0)
        {
            ...
            if (LoanInfoData != null)
            {
                ...
                this.txtRentAmount.Text = LoanInfoData.Rows[0]["RentAmount"] == DBNull.Value ? string.Empty : Convert.ToDecimal(LoanInfoData.Rows[0]["RentAmount"]).ToString("n2");
            }
    ...
    private void BindingData_LoanInfo(DataTable LoanInfoData)
    {
        ...
        this.txtAmount.Text = LoanInfoData.Rows[0]["LoanAmount"] == DBNull.Value ? string.Empty : Convert.ToDecimal(LoanInfoData.Rows[0]["LoanAmount"]).ToString("n2");
        this.txtRate.Text = LoanInfoData.Rows[0]["Rate"].ToString();
        this.txtPMI.Text = LoanInfoData.Rows[0]["MonthlyPMI"] == DBNull.Value ? string.Empty : Convert.ToDecimal(LoanInfoData.Rows[0]["MonthlyPMI"]).ToString("n2");
        this.txtPMITax.Text = LoanInfoData.Rows[0]["MonthlyPMITax"] == DBNull.Value ? string.Empty : Convert.ToDecimal(LoanInfoData.Rows[0]["MonthlyPMITax"]).ToString("n2");
        ...
        {           
            if (t_status = decimal.TryParse(this.txtRate.Text, out dRate))
            {
                this.txtRate.Text = dRate.ToString("n3");
            }           
        }
        ...
        this.txt2ndAmount.Text = LoanInfoData.Rows[0]["TD_2Amount"] == DBNull.Value ? string.Empty : Convert.ToDecimal(LoanInfoData.Rows[0]["TD_2Amount"]).ToString("n2");
    ...
    private void BindingData_IncomeAndEmployment()
    {
        ...
        {
            this.txtMonthlySalary.Text = BorrowerInfo_Income.Rows[0]["Salary"] == DBNull.Value ? string.Empty : Convert.ToDecimal(BorrowerInfo_Income.Rows[0]["Salary"]).ToString("n2");
            this.txtOtherMonthlyIncome.Text = BorrowerInfo_Income.Rows[0]["Other"] == DBNull.Value ? string.Empty : Convert.ToDecimal(BorrowerInfo_Income.Rows[0]["Other"]).ToString("n2");
        }

        if (this.BorrowerInfo_Assets != null)
        {
            this.txtLiquidAssets.Text = BorrowerInfo_Assets.Rows[0]["Amount"] == DBNull.Value ? string.Empty : Convert.ToDecimal(BorrowerInfo_Assets.Rows[0]["Amount"]).ToString("n2");
        }
    }
    ...
    private bool SaveLead(out int iReturnFileId)
    {
        ...
        if (this.sActionMode == "Edit_HasLoanId_NoContactId" || this.sActionMode == "Edit_HasContactId_HasLoan")
        {
            ...
            if (this.CoBorrowerInfo_Contact == null || this.CoBorrowerInfo_Contact.Rows.Count == 0)    // create co-borrower  //gdc add   row==0  //20121101
            {
                ...
                #region 建立Co-Borrower和Loan的关系

                if (iNewCoBorrowerID != 0)
                {
                    Global_CoBorrowerID = iNewCoBorrowerID;
                    this.ContactMgr.LinkCoBorrowerToLoan(iCurrentLoanId, iNewCoBorrowerID);
                }
            ...
            if (bCreateNewLoan == true)
            {
                ...
                if (sFirstNameCoBorrower != string.Empty)
                {
                    try
                    {
                        ...
                        {
                            //iNewCoBorrowerID = this.CreateContactAndProspect(sFirstNameCoBorrower, sLastNameCoBorrower, sEmailCoBorrower, sCellPhoneCoBorrower, sHomePhoneCoBorrower, sWorkPhoneCoBorrower, sDOBCoBorrower, sSSNCoBorrower, string.Empty, string.Empty, sFICOScoreCoBorrower, sMailingStreetAddress1, sMailingStreetAddress2, sMailingCity, sMailingState, sMailingZip, sLeadSource, sReferralID);
                            iNewCoBorrowerID = Global_CoBorrowerID;
                        }
                        else
                        {
                            //iNewCoBorrowerID = this.CreateContactAndProspect(sFirstNameCoBorrower, sLastNameCoBorrower, sEmailCoBorrower, sCellPhoneCoBorrower, sHomePhoneCoBorrower, sWorkPhoneCoBorrower, sDOBCoBorrower, sSSNCoBorrower, string.Empty, string.Empty, sFICOScoreCoBorrower, string.Empty, string.Empty, string.Empty, string.Empty, string.Empty, sLeadSource, sReferralID);
    ...
    {
        int iFileId = 0;
        if (!this.CurrUser.userRole.Prospect.ToUpper().Contains("D"))
        {
            PageCommon.AlertMsg(this, "You don't have the privileges to save a lead.");
            return;
        }
        ...
        if (bSuccess == true)
        {
            PageCommon.WriteJsEnd(this, "Saved successfully.", "window.location.href=window.location.href");
        }
    }
    ...
    protected void btnCreateAnother_Click(object sender, EventArgs e)
    {
        ...
        if (bSuccess == true)
        {
            PageCommon.WriteJsEnd(this, "Created successfully.", PageCommon.Js_RefreshSelf);
        }
    }
