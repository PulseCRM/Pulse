...
<%@ Page Title="Prospect Detail View - Emails" Language="C#" AutoEventWireup="true"
    CodeBehind="LoanDetailEmailTab.aspx.cs" Inherits="LPWeb.Layouts.LPWeb.Prospect.LoanDetailEmailTab" %>
...
<head id="Head1" runat="server">
    <title>Prospect Detail View - Emails</title>
...
// ]]>
    ...
    <script type="text/javascript">
        ...
        function aSendEmail_onclick() {
            ...
            //alert("From: " + From);
            if (From == "0" || From == "2") {   // 0: processing loan detail, 2: prospect loan detail

                var LoanID = GetQueryString1("itemid");
                //alert("LoanID: " + LoanID);
            ...
            else if (From == "1") { // prospect detail

                var ProspectID = GetQueryString1("itemid");
            ...
            else if (From == "3") { // Partner Contact detail

                var ContactID = GetQueryString1("itemid");
                //alert("ProspectID: " + ProspectID);

                var RadomNum = Math.random();
                var RadomStr = RadomNum.toString().substr(2);
                var iFrameSrc = "../LoanDetails/EmailSendPopup.aspx?sid=" + RadomStr + "&ContactID=" + ContactID + "&CloseDialogCodes=window.parent.CloseGlobalPopup()";
        ...
        function ShowDialog_EmailLogAttachments(EmailLogID) {
            ...
            var divHeight = iFrameHeight + 40;
            var sFrom = "<%=strFrom %>";
            ShowGlobalPopup("Email Log Attachments", iFrameWidth, iFrameHeight, divWidth, divHeight, iFrameSrc);
            //      if (sFrom == "3") {
            //          ShowGlobalPopup("Email Detail", iFrameWidth, iFrameHeight, divWidth, divHeight, iFrameSrc);
            //      }
            //      else {
                ...
                <td>
                    <div id="tabs10">
                        <ul>
                            ...
                            <li id="current"><a id="aEmails" href="javascript:aEmails_onclick()"><span>Emails</span></a></li>
            ...
            <asp:GridView ID="gridList" runat="server" DataKeyNames="EmailLogId,EmailBody" EmptyDataText="There is no email in database."
                ...
                <Columns>
                    ...
                    <asp:TemplateField HeaderText="Attachments" SortExpression="Attachments">
                        <ItemTemplate>
                            <a href="javascript:ShowDialog_EmailLogAttachments('<%# Eval("EmailLogId") %>')"><%# Eval("Attachments")%></a>
                        </ItemTemplate>
                    </asp:TemplateField>
                </Columns>
            </asp:GridView>
